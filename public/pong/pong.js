var PONG = function (ctx, main_width, main_height, leftWinFunc, rightWinFunc, ballHitLeftPaddleFunc, ballHitRightPaddleFunc, leftPaddleMoveFunc, rightPaddleMoveFunc) {
	...
	this.resetBoardData = resetBoardData;
	this.setScores = function(s) {
		scores = s;
	};
	this.setStarttime = function(s) {
		startTime = s;
	};
	
	function resetBoardData() {
		
		var items = [135 + Math.random()*90];
	...
	function checkForWin() {
		if (ball.x + ball.radius < 0) {
			...
			rightWinFunc(game_board, ball);
			resetBoardData();
		} else if (ball.x - ball.radius > game_board.width) {
			scores[0] += 1;
			console.log('Left wins!');
			leftWinFunc(game_board, ball);
			resetBoardData();
		}
	}
	...
	function moveBall() {
		...
		if (ball.y - ball.radius < 0 && ball.angle == ball.o_angle) { // hit top wall
			...
			return ballHitLeftPaddleFunc(nbally, ball.angle);
			...
			return ballHitRightPaddleFunc(nbally, ball.angle);
		} else {
	...
	function movePaddles() {
		...
		var move = leftPaddleMoveFunc(moveLeftPlayerDown, moveLeftPlayerUp, ball, paddles[0], paddle, game_board);
		if (move) move();
		
		// machine learning right side ai
		var move = rightPaddleMoveFunc(moveRightPlayerDown, moveRightPlayerUp, ball, paddles[1], paddle, game_board);
		if (move) move();
	}
